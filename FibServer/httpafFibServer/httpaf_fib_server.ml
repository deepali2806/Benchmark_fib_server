...
let m = Array.make 1000 (Eio_domainslib_interface.MVar.create_empty ());;
let i = Atomic.make 0
...
let request_handler _ reqd =
  ...
  | "/" -> 
            ...
            let n = ((Atomic.get i) mod 5) in
            let _ = Eio_domainslib_interface.MVar.put (40 + n) ( m.(Atomic.get i)) in ();
...
let () = 
    ...
    let pool = T.setup_pool ~num_additional_domains:(num_domains - 1) () in
            T.run pool (fun () -> 
                ...
                while true do
                    if (Atomic.get r) < (Atomic.get i) then
                    begin
                      let start_c = Unix.gettimeofday () in
                      let a = T.async pool (fun _ -> 
                        let n = Eio_domainslib_interface.MVar.take (m.(Atomic.get r)) in
                        let v = fib_par pool (n) in
                        printf "\n%d Fib %d ans %d" (Atomic.get r) n v;
                      ) in
                      T.await pool a;
                      Atomic.incr r;
                      let stop_c = Unix.gettimeofday () in
                      Printf.printf "\n %d Computation time: %fs\n\n%!" ((Atomic.get r) - 1 ) (stop_c -. start_c)                
                    end
                done;
            );
