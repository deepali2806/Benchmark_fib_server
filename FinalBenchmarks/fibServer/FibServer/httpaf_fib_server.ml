...
let m = Array.make 100 (Eio_domainslib_interface.MVar.create_empty ());;
let i = Atomic.make 0
...
let request_handler _ reqd =
  ...
  | "/" -> 
           let mvar_cnt = (Atomic.get i) in
           (* Can be get n from request *)
           let _ = Eio_domainslib_interface.MVar.put (40 + mvar_cnt) ( m.(mvar_cnt)) in ();
         
            let response_ok = Response.create ~headers `OK in
            Reqd.respond_with_bigstring reqd response_ok text;
            Atomic.incr i;     
            traceln "\nEio Fibre %d Ends%!" mvar_cnt
  | "/exit" ->
    exit 0
  | _   ->
...
let () = 
    ...
    let pool = T.setup_pool ~num_additional_domains:(num_domains - 1) () in
            T.run pool (fun () -> 
                let r = ref 0 in
                while true do
                    (* If our count is < ith value *)
                    if !r < (Atomic.get i) then
                    begin
                      let a = T.async pool (fun _ -> 
                        let n = Eio_domainslib_interface.MVar.take (m.(!r)) in
